...
@Component({
  ...
  standalone: true,
  imports: [ToastModule, CommonModule, BadgeModule, DropdownModule, ContainerComponent, HeaderTogglerDirective, SidebarToggleDirective, IconDirective, HeaderNavComponent, NavItemComponent, NavLinkDirective, RouterLink, RouterLinkActive, NgTemplateOutlet, BreadcrumbRouterComponent, ThemeDirective, DropdownComponent, DropdownToggleDirective, TextColorDirective, AvatarComponent, DropdownMenuDirective, DropdownHeaderDirective, DropdownItemDirective, BadgeComponent, DropdownDividerDirective, ProgressBarDirective, ProgressComponent, NgStyle]
})
export class DefaultHeaderComponent extends HeaderComponent implements OnInit {
  ...
  reloadNotifications() {
    this.notification.all().subscribe(n => {
  ...
  constructor(private router: Router, private notification: NotificationService, public signalRService: SignalRService, private auth: Auth) {
    super();
    signalRService.setCallback((data: any) => {
      data = JSON.parse(data);
      if (data.UserId == auth.getUser().id || data.UserId == 0) {
        this.currentNotification = data.Title;
        this.toggleToast();
      }
    });
  }
